...
const TrackPage = () => {
  ...
  const { id: trackId } = useParams(); // Get track ID from URL params
  ...
  useEffect(() => {
          ...
          if (matrix && matrix.track) {
            ...
            if (track) {
              ...
              if (trackId !== matrix.track) {
                navigate(`/track/${matrix.track}`, { replace: true });
              }
            }
          } else if (matrix) {
            ...
            navigate(`/track/${newTrack.track_id}`, { replace: true });
          } else {
    ...
    loadTrackData();
  }, [trackId, currentMatrixId, navigate, user]);

  return (
    <div className="w-full h-[98vh] gap-2">
      <div className="flex my-2 h-[98vh] gap-2">
        <div className="w-full rounded-[8px] h-[98vh] relative overflow-hidden">
