...
const JoinMatrix = () => {
  const { matrixId } = useParams(); // Get matrixId from URL params
  ...
  const handleJoinMatrix = async () => {
    ...
    try {
      ...
      navigate(`/matrix/${matrixId}`); // Changed from router.push
    } catch (err) {
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full mx-4">
        ...
        {user ? (
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => navigate(`/login?redirect=/join/${matrixId}`)}
